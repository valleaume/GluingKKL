...
% Assume X is your input data (N x D), Y is your output data (N x P)
P = size(Y_train, 2); % Number of output dimensions
mdl = cell(P, 1);

gpOptions = struct(...
    'KernelFunction', 'matern32', ...
    'FitMethod', 'fic', ... % Faster for large datasets
    'PredictMethod', 'bcd', ... % Faster predictions
    'Standardize', true);

% Convert to name-value pairs
nvPairs = {};
fields = fieldnames(gpOptions);
for i = 1:numel(fields)
    field = fields{i};
    nvPairs = [nvPairs, {field, gpOptions.(field)}];
end


for i = 1:P
    mdl{i} = fitrgp(X_train, Y_train(:, i), nvPairs{:});
    disp(['Trained GP for output dimension ' num2str(i) '/' num2str(P)]);
end
%mdl = fitrgp(X_train, Y_train(:, 1), 'KernelFunction', 'squaredexponential', 'Standardize', true);

Y_pred = zeros(size(Y_test));
for i = 1:P
    Y_pred(:, i) = predict(mdl{i}, X_test);
end
...
model_filename = strcat('ObserverModels/dc-motor-predictor-GP-', today);
save(model_filename, 'mdl', 'A', 'B', 'dataset_name');
